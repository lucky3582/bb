...
void process_data(char* buffer, long long bufferSizeInBytes){
    printf("In Reading: %p\n",*&buffer);
    printf("In Reading: %lld",bufferSizeInBytes);
...
void remap()
{
char*  new = mremap(ptr,SIZE,SIZE+1024,MREMAP_MAYMOVE);
if(new == MAP_FAILED){
...
if(ptr+bufferSizeInBytes >ptr+1024)
